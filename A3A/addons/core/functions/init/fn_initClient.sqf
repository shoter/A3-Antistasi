...
flagx addAction [localize "STR_A3A_fn_init_initClient_addAct_recruitSquad", { createDialog "A3A_RecruitSquadDialog"; },nil,0,false,true,"","([_this] call A3A_fnc_isCommandStaff) and !A3A_petrosMoving",4];
flagX addAction [localize "STR_A3A_fn_init_initClient_addAct_teleportFlag", { createDialog "A3A_DeployedFlagDialog"; },nil,0,false,true,"","!isNull A3A_deployedFlag and (side group _this == teamPlayer) and (vehicle _this == _this) and (_this == _this getVariable ['owner',objNull]) and !A3A_petrosMoving",4];
flagX addAction [localize "STR_A3A_fn_init_installClientEH_removeFlag", { [player] remoteExecCall ["A3A_fnc_deployedFlagRemove", 2]; },nil,0,false,true,"","(_this == theBoss) and !isNull A3A_deployedFlag and !A3A_petrosMoving",4];
