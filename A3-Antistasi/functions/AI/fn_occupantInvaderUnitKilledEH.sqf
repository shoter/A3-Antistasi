...
if (side (group _killer) == teamPlayer) then
{
	...
	if (count weapons _victim < 1 && !(_victim getVariable ["isAnimal", false])) then
    {
		...
		if (_victimSide == Occupants) then
		{
			...
			[[20, 30], [0, 0]] remoteExec ["A3A_fnc_addAggro",2];
		}
		else
		{
			[[0, 0], [20, 30]] remoteExec ["A3A_fnc_addAggro",2];
		};
	}
	else
	{
		...
		if (_victimSide == Occupants) then
		{
			[[0.5, 45], [0, 0]] remoteExec ["A3A_fnc_addAggro",2];
		}
		else
		{
			[[0, 0], [0.5, 45]] remoteExec ["A3A_fnc_addAggro",2];
		};
	};
}
else
